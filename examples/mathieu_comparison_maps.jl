...
# 1. Utility to create problem (MFCM style)
...
function run_comparison_maps()
    ...
    # Map 1: delta vs b0
    a1 = 0.01; ε = 1.0; T_period = π;  p_steps_SD = 40;p_steps_MFCM = 10
    foo_mfcm(δ, b0) = log(mfcm_mumax(δ, ε, b0, a1; T=T_period, p_steps=p_steps_MFCM, tableau=GL(2)))
    τmax = 2π
    ...
    println("Running MDBM for Map 1 (MFCM)...")
    ...
    if !isempty(pts_mfcm)
        scatter!(p1, pts_mfcm[1, :], pts_mfcm[2, :], markersize=2, label="MFCM (GL2)", color=:red)
    end
    ...
    b0 = 0.0; T_period = 2π; p_steps_SD = 40;p_steps_MFCM = 10
    
    foo_mfcm2(δ, ε_val) = log(mfcm_mumax(δ, ε_val, b0, a1; T=T_period, p_steps=p_steps_MFCM, tableau=GL(2)))
    foo_sdm2(δ, ε_val) = log(sdm_mumax(δ, ε_val, b0, a1; T=T_period, p_steps=p_steps_SD))
    
    axis2 = [Axis(-2.0:1.0:5.0, :δ), Axis(-0.01:1.0:5.0, :ε)]
    
    println("Running MDBM for Map 2 (MFCM)...")
    ...
    if !isempty(pts_mfcm2)
        scatter!(p2, pts_mfcm2[1, :], pts_mfcm2[2, :], markersize=2, label="MFCM (GL2)", color=:red)
    end
    ...
    T_period = 2π; ; p_steps_SD = 40;p_steps_MFCM = 10
    
    foo_mfcm3(δ, b0_val, ε_val) = log(mfcm_mumax(δ, ε_val, b0_val, a1; T=T_period, p_steps=p_steps_MFCM, tableau=GL(2)))
    ...
    println("Running MDBM for Map 3 (MFCM)...")
    ...
    if !isempty(pts_mfcm3)
        scatter!(p3, pts_mfcm3[1, :], pts_mfcm3[2, :], pts_mfcm3[3, :], markersize=0.5, label="MFCM (GL2)", color=:red)
    end
